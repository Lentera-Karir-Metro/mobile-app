...
class _ExploreScreenState extends State<ExploreScreen> {
  ...
  void _showFilterModal() {
    ...
    final categories = ['All', ...catalogProvider.categories.map((c) => c.name)];
                  ...
                  : GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        if (course.discountPrice != null && course.discountPrice! > 0) {
                          ...
                          final finalPrice = course.price - course.discountPrice!;
                          priceText = 'Rp${finalPrice.toStringAsFixed(0).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (m) => '${m[1]}.')}';
                          originalPrice = 'Rp${course.price.toStringAsFixed(0).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (m) => '${m[1]}.')}';
                        } else {
                          // No discount
                        ...
                        return CourseCard(
                          thumbnailPath: course.thumbnail ?? 'assets/hardcode/sample_image.png',
                          title: course.title,
                          price: priceText,
                          originalPrice: originalPrice,
                          mentorName: course.mentor?.name ?? course.instructor,
